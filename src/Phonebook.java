...
//Changes: Phone Number over 10 digits fix

//TBD Changes: 
// - Textfields in an array
// - Folders/Customizable Categories for organizing contacts
// - Separate window for viewing/entering contacts
...
public class Phonebook extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if(evt.getActionCommand().equals("Search Select")){
			...
			if((cbxSearchType.getSelectedIndex() >=0)&&cbxSearchType.getSelectedIndex()<8){
				txtFldSearch.setEnabled(true);
				txtFldSearch.setVisible(true);
				cbxGenderSearch.setVisible(false);
				cbxProvinceSearch.setVisible(false);
			}
			if(cbxSearchType.getSelectedIndex() == 8){
				cbxProvinceSearch.setSelectedIndex(-1);
		...
		try{
			...
			try{
				...
				try{
					...
					try{
						if(!txtFldHomeNumber.getText().equals("")){
							if((Long.parseLong(txtFldHomeNumber.getText())/1000000000 !=0)&&(Long.parseLong(txtFldHomeNumber.getText())/1000000000 <10)){
						...
						if(!txtFldMobileNumber.getText().equals("")){
							if((Long.parseLong(txtFldMobileNumber.getText())/1000000000 !=0)){
								long temp = Long.parseLong(txtFldMobileNumber.getText());
								if (editing == false)
									resizePhonebook();
								if (editing == true){ 
									validEdits = true;
